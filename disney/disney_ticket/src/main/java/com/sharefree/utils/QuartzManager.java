...
public class QuartzManager {
	 ...
	 */
	public static void addSimpleJob(String jobName, String jobGroupName, String triggerName, String triggerGroupName, Class<? extends Job> jobClass,
			Date startTime, Date endTime, int repeatCount, int repeatInterval, String calName) {
		...
		SimpleTrigger trigger = newTrigger().withIdentity(triggerName, triggerGroupName).startAt(startTime == null ? getStartTime() : startTime)
				.withSchedule(simpleSchedule().withRepeatCount(repeatCount).withIntervalInSeconds(repeatInterval)).modifiedByCalendar(calName).build();
		try {
	 ...
	 */
	public static void addCronJob(String jobName, String jobGroupName, String triggerName, String triggerGroupName, Class<? extends Job> jobClass,
		...
		if (StringUtil.isEmpty(triggerGroupName))
			triggerGroupName = DEFAULT_TRIGGER_GROUP;
		JobDetail jobDetail = newJob(jobClass).withIdentity(jobName, jobGroupName).build();
		CronTrigger trigger = newTrigger().withIdentity(triggerName, triggerGroupName).startAt(startTime == null ? getStartTime() : startTime)
		...
		try {
			...
			if (oldRepeatInterval != newRepeatInterval) {
				SimpleTrigger newTrigger = newTrigger().withIdentity(triggerKey)
						.withSchedule(simpleSchedule().withIntervalInSeconds(newRepeatInterval).withRepeatCount(trigger.getRepeatCount()))
						.modifiedByCalendar(calName).build();
	...
	public static void updateCronJob(String triggerName, String triggerGroupName, String newCronExpression) {
		try {
			...
			if (oldCronExpression != newCronExpression) {
				CronTrigger newTrigger = newTrigger().withIdentity(triggerKey).withSchedule(cronSchedule(newCronExpression)).build();
				scheduler.rescheduleJob(triggerKey, newTrigger);
			}
		} catch (Exception e) {
			throw new RuntimeException(e);
	 ...
	 * @param calendarName
	 */
	public static void removeJob(String jobName, String jobGroupName, String triggerName, String triggerGroupName, String calendarName) {
		try {
			if (StringUtil.isEmpty(jobGroupName))
				jobGroupName = DEFAULT_JOB_GROUP;
			if (StringUtil.isEmpty(triggerName))
